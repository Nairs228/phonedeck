...
Или прописать в systemd-сервис (см. раздел [Перенос на новый хостинг](#перенос-на-новый-хостинг)).
...
1. Указать WiFi и адрес сервера в `arduino/NodeMCU_updated/NodeMCU_updated.ino`:
   ```cpp
...
2. **Пароль администратора** задан в коде (`app.py`, константа `DEFAULT_ADMIN_PASSWORD`). При каждом старте сервера он сбрасывается к значению из кода. Для смены пароля нужно либо изменить константу в коде, либо реализовать отдельный механизм смены пароля.
